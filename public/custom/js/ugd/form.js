    ...
    $.ajax({
       ...
       success: function msg(res){
            ...
            if (status==true) {
             ...
             disableForm();
             window.location = global_url+"ugd/";
            }
       }
    }); 
}
    ...
    $.ajax({
       ...
       success: function msg(res){
            ...
            if (status==true) {
             ...
             $('input[name=txtIdRekmedDetail]').val(idDetailRekmed);
             disableForm();
             var txtPengobatan = $('#txtPengobatan').val();
             if(txtPengobatan!=""){
                 var jenisApotik = $('#selectApotik').val();
                 updateStatusApotikUGD("apotik" , 1 , jenisApotik);
             }
             updateStatusAntrian("poli" ,"3");
            }
       }
    }); 
}
...
function updateStatusApotikUGD(pelayanan, status, statusJenisApotik){
    $.ajax({
       url : base_url_register+"update-status-antrian/",
       type : "POST",
       data : "idKunjungan="+$('input[name=txtIdKunjungan]').val()+"&statusKunjungan="+status+"&statusJenisApotik="+statusJenisApotik+"&statusPelayanan=apotik",
       dataType : "html",
       success: function msg(res){
            var data = jQuery.parseJSON(res);
            var status = data['status'];
