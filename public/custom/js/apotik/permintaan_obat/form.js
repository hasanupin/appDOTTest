...
$(function() {  
  ...
  $('#btnCariObat').click(function() {
    $.ajax({
      url : global_url + 'apotik/'+$('#form-permintaan').data('apotik')+'/permintaan_obat/cari_obat?key=' + $('#txtObat').val(),
...
/*function tambahObat(obat) {  
  $('#txtObat').val('');
  $.ajax({
    url : global_url + 'apotik/'+$('#form-permintaan').data('apotik')+'/permintaan_obat/tambah_obat',
    type : 'post',
    data : 'obat=' + obat,
    dataType : 'json',
    success : function(response) {
