...
$(function() {  
  ...
  $('#btnCariObat').click(function() {
    $.ajax({
      url : global_url + 'apotik/ugd/penyesuaian/stok/cari_obat?key=' + $('#txtObat').val(),
...
/*function tambahObat(obat) {  
  $('#txtObat').val('');
  $.ajax({
    url : global_url + 'apotik/ugd/penyesuaian/stok/tambah_obat',
    type : 'post',
    data : 'obat=' + obat,
    dataType : 'json',
    success : function(response) {
...
function getObat(id) {
  $.ajax({
    url : global_url +'apotik/ugd/penyesuaian/stok/get_obat/' + $('[name="obat['+id+'][id_obat]"]').val(),
    success : function(response) {      
