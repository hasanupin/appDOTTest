...
$(function() {  
  ...
  $('#btnCariObat').click(function() {
    $.ajax({
      url : global_url + 'gudang_apotik/penyesuaian/stok/cari_obat?key=' + $('#txtObat').val(),
...
function getObat(id) {
  $.ajax({
    url : global_url +'gudang_apotik/penyesuaian/stok/get_obat/' + $('[name="obat['+id+'][id_obat]"]').val(),
    success : function(response) {      
...
/*function tambahObat(obat) {  
  $('#txtObat').val('');
  $.ajax({
    url : global_url + 'gudang_apotik/penyesuaian/stok/tambah_obat',
    type : 'post',
    data : 'obat=' + obat,
    dataType : 'json',
    success : function(response) {
      if (response.length > 1) {
        ...
        if (response[0]) {                              
          ...
          html = '<tr data-row="'+rowsObat+'">';
            ...
            html +='<td><input type="text" name="obat['+rowsObat+'][jml]" value="'+response[0]['stok']+'" class="form-control text-center input-sm" readonly/></td>';      
