...
$(function() {  
  ...
  $('#btnCariObat').click(function() {
    $.ajax({
      url : global_url + 'gudang_apotik/'+$('#form-penerimaan').data('apotik')+'/penerimaan/obat/cari_obat?key=' + $('#txtObat').val(),
...
/*function tambahObat(obat) {  
  $('#txtObat').val('');
  $.ajax({
    url : global_url + 'gudang_apotik/'+$('#form-penerimaan').data('apotik')+'/penerimaan/obat/tambah_obat',
    type : 'post',
    data : 'obat=' + obat,
    dataType : 'json',
    success : function(response) {
